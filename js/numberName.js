 ...
 */
function numberName(n, abbreviate, onlyExp, decimalPlaces = 2) { // i made something like this a while back, then remade it from stratch, and this is the third iteration and it is so much more optimized like idk what on earth i was doing before
    ...
    if (type === "number" && isNaN(n)) return "NaN";
    ...
    if (type === "boolean") {
        ...
        return numberName(String(n), abbreviate, onlyExp, decimalPlaces);
    } else if (type === "bigint") {
        exponent = n.toString().length - 1;
        mantissa = n.toString()[0] + "." + n.toString().slice(1);
        if (n < 0n) sign = "-";
    } else if (type === "string") {
        ...
        throw new TypeError("Invalid number input: ", n);
    }

    if (isNaN(Number(exponent))) return "NaN";
    ...
    if (!onlyExp && !abbreviate) {
        ...
        if (!c[1]) c[1] = "0";
        name += c[0] + (c[1] ? "." + c[1].slice(0, Math.min(decimalPlaces, c[1].length)) : "");
        if (decimalPlaces === 0 && name.includes(".")) name = name.slice(0, name.indexOf("."));
        if (c[1] && c[1].length < decimalPlaces) {
            name += "0".repeat(decimalPlaces - c[1].length);
        }
        name += " ";

        if (c.includes("1000")) { // handle cases such as 999,999,999 from being shown as "1000.00M"
            name = name.replace("1000", "1");
            exponent += 3n;
        }
    }
    ...
    if (exponent === 3n) {
        ...
        if (exponent < 33n) {
            ...
            if (exponent >= 3003n) {
                name += numberName(`1e${(exponent - 3n) / 1000n + 3n}`, abbreviate, onlyExp, 0)
                    .replace("one hundred ", "")
                    .replace("ten ", "")
                    .replace("one ", "")
                    .replace("100 ", "")
                ...
                name += numberName(`1e${(exponent - 3n) % 3000n + 3n}`, abbreviate, onlyExp, 0)
    ...
    if (exponent === -Infinity || mantissa === 0 && !onlyExp) name = "zero";
